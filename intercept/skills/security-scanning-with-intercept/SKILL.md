...
- **Report EVERY real finding, at ALL severities — Critical, High, Medium, Low, AND Unknown. Do NOT limit the scan to criticals/highs.** A scan that returns only the criticals is **wrong** — it hides most of the repo's real risk surface and misrepresents its security posture. A legitimate Medium or Low is a **must-report**, not noise: report it with honest severity. Completeness across the full severity range is the goal, every scan.
- **Cover the full spectrum — simple to advanced, and Critical down to Low.** Report the basic/obvious issues for completeness *and* the deep, expert-level ones for insight. Don't skip the simple stuff, and don't stop at the criticals.
- **The filter is "real vs. speculative" — NEVER "high-severity vs. low-severity."** Report what you can **stand behind**; drop only what you genuinely *can't substantiate*. A flood of speculative findings is bad — but dropping a real Low because "it's only a Low" is just as bad. "Quality > volume" means *substantiated*, not *severe*. AI findings are first-class (they move the Intercept Score and compliance), so report every real one regardless of severity.
- When you're genuinely unsure a finding is real, lower its severity and frame it as "review/verify," or omit it. Don't assert a `CRITICAL` you can't substantiate — and equally, don't drop a real `LOW` just because it's low.
- Severity honestly: `CRITICAL` = exploitable now, real impact; `HIGH` = serious, conditional; `MEDIUM`/`LOW` = real but lower impact or likelihood; `UNKNOWN` = real but you can't pin the severity. **Every one of these belongs in the report — including the Mediums, Lows, and Unknowns.**
